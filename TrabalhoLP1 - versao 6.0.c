...
void Adicionar_Contato(contato **lista) // FUNÇÃO ADD CONTATO
{
	...
	strcpy(novo_contato->numero_telefone, verificar_contato);
...
void Remover_Contato(contato **lista) // FUNÇÃO DEL CONTATO
{
	...
	while(anterior != NULL){
		...
		if(strcmp(atual->numero_telefone, num_tel)==0)
		{
			...
			{
				printf("strcmp anterior 2222 tel\n");
				aux = 1;
				*lista = anterior->prox;
				free(anterior);
				break;
...
{
    int opc, saida = 0, mesmo = 1, aux = 0;
    char num_tel[MAX_TEL], identificar_telefone[MAX_TEL];
